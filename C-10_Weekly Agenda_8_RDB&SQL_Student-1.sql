...
SELECT A.brand_name, YEAR(D.order_date) AS [year], SUM(C.quantity) AS [annual amount of product]
FROM product.brand A, product.product B, sale.order_item C, sale.orders D
WHERE A.brand_id = B.brand_id
AND B.product_id = C.product_id
AND C.order_id = D.order_id --yılı bağlamayı unuttum farklı bir hata verdi
GROUP BY A.brand_name, YEAR(D.order_date)
ORDER BY A.brand_name, [year];
